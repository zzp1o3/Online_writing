...
import type { ChapterApproval } from "../models/approval.js";
import type { WriteMode, ProjectConfig } from "../models/project.js";
...
import { retrieveForTiming, type TimingRetrievalResult } from "../memory/retrieval.js";
...
export class PipelineRunner {
  ...
  // ---------------------------------------------------------------------------
  // 守护检查 + 人工审批 + 局部重启 + 向量索引（设计文档 §5 / §6 / §9）
  // ---------------------------------------------------------------------------

  /** 读取项目配置（解析 inkos.json；解析失败返回 null，不阻塞流水线） */
  private async readProjectConfig(): Promise<ProjectConfig | null> {
    try {
      const raw = await this.state.loadProjectConfig();
  ...
  /** 守护检查（T3：每章开头伏笔/卡片/戏份检查，设计文档 §6.3） */
  async runGuardianCheck(
    ...
    const guardian = new GuardianAgent(this.agentCtxFor("guardian", bookId));
    return guardian.guard({
      book: { title: book.title, genre: book.genre },
      bookDir,
      chapterNumber,
      chapterIntent,
      sideCharacterThresholds: project?.branch
        ? {
            warn: project.branch.sideCharacterWarnRatio,
  ...
  /** 时机化检索（T1/T2/T4/T5，设计文档 §6.3），供外部工作台/Agent 调用 */
  async retrieveMemoryByTiming(params: {
    readonly bookId: string;
    readonly timing: "chapter-plan" | "pre-write" | "guardian-check" | "judge" | "branch-op";
    readonly query: string;
  }): Promise<TimingRetrievalResult> {
    const project = await this.readProjectConfig();
    return retrieveForTiming({
      bookDir: this.state.bookDir(params.bookId),
      timing: params.timing,
      query: params.query,
      embeddingConfig: project?.embedding,
    });
  }

  /** 用户点头通过章节（设计文档 §9）：记录审批 + 更新章节状态 + 建立向量索引 */
  async approveChapter(bookId: string, chapterNumber: number, comment = ""): Promise<ChapterApproval> {
    const bookDir = this.state.bookDir(bookId);
    const approval = await transitionChapterApproval(bookDir, chapterNumber, "approved", "user", comment);
    const index = await this.state.loadChapterIndex(bookId);
    const updated = index.map((ch) =>
      ch.number === chapterNumber ? { ...ch, status: "approved" as const } : ch,
    );
    await this.state.saveChapterIndex(bookId, updated);
    await this.indexChapterIntoVector(bookId, chapterNumber).catch((error) => {
      this.config.logger?.warn(`[approve] 向量索引失败（不影响审批）: ${String(error)}`);
    });
    return approval;
  }

  /** 整章重来（设计文档 §9）：回滚到上一章并记录 rejected_whole */
  async rejectChapterWhole(
    bookId: string,
    chapterNumber: number,
    reason = "",
  ): Promise<{ readonly approval: ChapterApproval; readonly rolledBackTo: number; readonly discarded: ReadonlyArray<number> }> {
    const bookDir = this.state.bookDir(bookId);
    const rollbackTarget = chapterNumber - 1;
    const discarded = await this.state.rollbackToChapter(bookId, rollbackTarget);
    const approval = await transitionChapterApproval(bookDir, chapterNumber, "rejected_whole", "user", reason);
    await removeVectorEntries(bookDir, (entry) =>
      entry.source === "chapter" && Number(entry.ref) >= chapterNumber,
    ).catch(() => undefined);
    return { approval, rolledBackTo: rollbackTarget, discarded };
  }

  /** 局部重启（设计文档 §9）：用户划定范围 + 指令，Agent 从全局视角重写该段落 */
  async partialRestartChapter(params: {
    readonly bookId: string;
    readonly chapterNumber: number;
    readonly range: readonly [number, number];
    readonly instruction: string;
  }): Promise<ChapterPipelineResult> {
    const releaseLock = await this.state.acquireBookLock(params.bookId);
    try {
      return await this._partialRestartChapterLocked(
        params.bookId,
  ...
  private async _partialRestartChapterLocked(
    bookId: string,
    chapterNumber: number,
    ...
    const bookDir = this.state.bookDir(bookId);
    const stageLanguage = await this.resolveBookLanguage(book);
    const bookDir2 = bookDir;
    const { title: chapterTitle, content: current } = await this.readChapterWithTitle(bookDir2, chapterNumber);
    if (range[0] < 0 || range[1] > current.length || range[0] >= range[1]) {
      throw new Error(`局部重启范围无效：章节长度 ${current.length}，范围 [${range[0]}, ${range[1]}]`);
    }
    const spec = {
    ...
    await attachPartialRestart(bookDir2, chapterNumber, spec);

    this.logStage(stageLanguage, {
      zh: `局部重启第 ${chapterNumber} 章 [${range[0]}-${range[1]}]`,
      en: `partial restart chapter ${chapterNumber} [${range[0]}-${range[1]}]`,
    });

    // 归档当前版本，供回退
    await archiveChapterVersion(bookDir2, chapterNumber, current, "regeneration");

    // 全局上下文检索（T4/T2 混合）：相关历史正文 + 设定卡
    const project = await this.readProjectConfig();
    const timing = await retrieveForTiming({
      bookDir: bookDir2,
      timing: "pre-write",
      query: instruction,
      embeddingConfig: project?.embedding,
    });
    const retrievalContext = timing.hits.length > 0
      ? timing.hits.map((h) => `## ${h.entry.source} (${h.entry.ref})\n${h.entry.text}`).join("\n\n")
      : "";

    const reviser = new ReviserAgent(this.agentCtxFor("reviser", bookId));
    const rewritePrompt = [
      `# 局部重启任务（第 ${chapterNumber} 章）`,
      `范围：字符 [${range[0]}, ${range[1]}]`,
      `用户指令：${instruction}`,
      ``,
      `要求：从全局视角重写指定范围内的内容，保持章节其余部分不动；`,
      `重写必须衔接前后文，尊重既有设定卡片与伏笔。`,
      retrievalContext ? `\n## 相关记忆\n${retrievalContext}` : "",
    ].join("\n");
    ...
    const revised = await reviser.reviseChapter(
      ...
      [{
        severity: "warning",
        category: "partial-restart",
        description: `局部重启：${instruction}`,
        suggestion: rewritePrompt,
      }],
      "rewrite",
      book.genre,
    );

    const finalContent = revised.revisedContent;
    const finalWordCount = countChapterLength(finalContent, stageLanguage === "en" ? "en_words" : "zh_chars");
    await archiveChapterVersion(bookDir2, chapterNumber, finalContent, "revision");
    await this.writeChapterWithTitle(bookDir2, chapterNumber, chapterTitle, finalContent);

    const auditResult = {
      passed: true,
      issues: [] as ReadonlyArray<AuditIssue>,
      summary: `局部重启完成：${instruction}`,
    };
    const status = "ready-for-review" as const;
    await transitionChapterApproval(bookDir2, chapterNumber, "ai_reviewed", "ai", `局部重启完成`);
    await setJudgeSummary(bookDir2, chapterNumber, auditResult.summary);

    return {
      chapterNumber,
      title: (await this.state.loadChapterIndex(bookId)).find((c) => c.number === chapterNumber)?.title ?? `第 ${chapterNumber} 章`,
  ...
  /** 记录章节审批台账（设计文档 §9）：按写入模式决定是否进入人工审批 */
  private async recordChapterApproval(
    bookId: string,
    chapterNumber: number,
    resolvedStatus: "ready-for-review" | "audit-failed" | "state-degraded",
    ...
    try {
      const project = await this.readProjectConfig();
      const writeMode: WriteMode = project?.writeMode ?? "direct";
      let state: import("../models/approval.js").ApprovalState;
      if (resolvedStatus === "state-degraded") {
        state = "draft";
      } else if (resolvedStatus === "audit-failed" || !auditResult.passed) {
        state = "ai_reviewed";
      } else if (writeMode === "approve" || (this.config.chapterReviewMode ?? "auto") === "manual") {
        state = "human_pending";
      } else {
        state = "approved";
      }
      await transitionChapterApproval(
        bookDir,
        chapterNumber,
        state,
        "ai",
        revised ? "AI 审判+修订完成" : "AI 审判完成",
      );
      await setJudgeSummary(bookDir, chapterNumber, auditResult.summary);
      if (state === "approved") {
        await this.indexChapterIntoVector(bookId, chapterNumber).catch(() => undefined);
      }
    } catch (error) {
      this.config.logger?.warn(`[approval] 记录审批失败（不阻塞流水线）: ${String(error)}`);
    }
  }

  /** 将已通过章节写入向量索引（正文 + 摘要，每章 1 个 chunk，设计文档 §6.2） */
  async indexChapterIntoVector(bookId: string, chapterNumber: number): Promise<void> {
    const bookDir = this.state.bookDir(bookId);
    const content = await this.readChapterContent(bookDir, chapterNumber);
    if (!content.trim()) return;
    const project = await this.readProjectConfig();
    const summary = await this.readChapterSummary(bookDir, chapterNumber);
    await upsertVectorEntries({
      ...
      entries: [{
        ...
        source: "chapter",
        ref: String(chapterNumber),
        text: buildChapterChunkText(content, summary),
      }],
    });
  }

  /** 读取章节摘要（chapter_summaries.md），找不到返回空串 */
  private async readChapterSummary(bookDir: string, chapterNumber: number): Promise<string> {
    try {
      const raw = await readFile(join(bookDir, "story", "chapter_summaries.md"), "utf-8");
      const lines = raw.split(/\r?\n/);
      const idx = lines.findIndex((line) => /^\|?\s*\d+\s*\|/.test(line) && line.includes(String(chapterNumber)));
      if (idx >= 0) return lines[idx] ?? "";
      return "";
    } catch {
      return "";
    }
  ...
  /** 局部重启：读取章节正文并保留标题行 */
  private async readChapterWithTitle(bookDir: string, chapterNumber: number): Promise<{ readonly title: string; readonly content: string }> {
    const chaptersDir = join(bookDir, "chapters");
    const files = await readdir(chaptersDir);
    const paddedNum = String(chapterNumber).padStart(4, "0");
  ...
  /** 局部重启：写回章节正文（保留标题行） */
  private async writeChapterWithTitle(bookDir: string, chapterNumber: number, title: string, content: string): Promise<void> {
    const chaptersDir = join(bookDir, "chapters");
    const files = await readdir(chaptersDir);
    const paddedNum = String(chapterNumber).padStart(4, "0");
    const chapterFile = files.find((f) => f.startsWith(paddedNum) && f.endsWith(".md"));
    ...
    await writeFile(join(chaptersDir, chapterFile), `${title}\n\n${content}`, "utf-8");
  }

  /** 分支操作（设计文档 §7）：新建/更新/砍分支；砍分支时返回受影响章节区间用于回滚重写 */
  async saveBranchGraph(bookId: string, graph: BranchGraph): Promise<void> {
    await saveBranchGraph(this.state.bookDir(bookId), graph);
  }

  async loadBranchGraphForBook(bookId: string): Promise<BranchGraph> {
    return loadBranchGraph(this.state.bookDir(bookId));
  }

  async cutBranchForBook(
    bookId: string,
    nodeId: string,
    reason: string,
  ): Promise<{ readonly cutNodeIds: ReadonlyArray<string>; readonly affectedChapterRange: { readonly min: number; readonly max: number } | null }> {
    const result = await cutBranch(this.state.bookDir(bookId), nodeId, reason);
    return { cutNodeIds: result.cutNodeIds, affectedChapterRange: result.affectedChapterRange };
  }

  /** 卡片操作（设计文档 §8）：仅 guardian / user 可写 */
  async listCards(bookId: string): Promise<import("../models/cards.js").CardsBundle> {
    return loadCards(this.state.bookDir(bookId));
  }

  async saveCard(bookId: string, card: StoryCard, editor: "guardian" | "user"): Promise<void> {
    await upsertCard(this.state.bookDir(bookId), card, editor);
  }
  ...
  private async _writeNextChapterLocked(
    ...
    // 6.5 审批台账记录（设计文档 §9）
    await this.recordChapterApproval(bookId, chapterNumber, resolvedStatus, auditResult, revised);
